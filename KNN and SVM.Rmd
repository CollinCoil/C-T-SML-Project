...
carEmissions$`Model Year` <- as.factor(carEmissions$`Model Year`)
carEmissions$`Regulatory Class` <- as.factor(carEmissions$`Regulatory Class`)
carEmissions$`2-Cycle MPG`<- as.numeric(carEmissions$`2-Cycle MPG`)

carEmissions$Quartile<-cut(carEmissions$`Real-World CO2 (g/mi)`,quantile(carEmissions$`Real-World CO2 (g/mi)`),include.lowest=TRUE,labels=FALSE)

carEmissionsTrimmed <- carEmissions %>% 
                          select(-`Production (000)`, -`Production Share`, -`Real-World CO2 (g/mi)`,
...
Xtrain <-  data.frame(carEmissions[Z,]) 
Ytrain <-  data.frame(carEmissions$Quartile[Z])
Xtest <-  carEmissions[-Z,-4] 
Yhat <- knn(Xtrain, Xtest, Ytrain, k = 25) # THIS IS CAUSING THE ERROR
Ytest <- carEmissions$Quartile[-Z,]
